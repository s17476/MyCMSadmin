...
class ContentAdminPanel extends Component {
    ...
    async componentDidMount() {
        ...
        var contentRef = await dataRef.collection("pages").doc(localStorage.getItem("item")).collection("style").doc("title");
        contentRef.get()
            .then(async (doc) => {
                var contentImgRef = await dataRef.collection("pages").doc(localStorage.getItem("item")).collection("style").doc("image");
                contentImgRef.get()
                    .then(async (docImg) => {
                                                ...
                                                <button className="App-add" id="AddPage"
                                                        onClick={async () => {
                                                            ...
                                                            if (newPage == null || newPage == "") {
                                                                ...
                                                                await dataRef.collection("pages").doc(newPage).set({
                                                                ...
                                                                await dataRef.collection("pages").doc(newPage).collection("style").doc("image").set({
                                                                    float: "left",
                                                                    height: "200px",
                                                                    width: "200px"
                                                                }, {merge: true})
                                                                ...
                                                                await dataRef.collection("pages").doc(newPage).collection("style").doc("text").set({
                                                                    color: "#055dff",
                                                                    fontSize: "30",
                                                                    margin: "0"
                                                                }, {merge: true})
                                                                ...
                                                                await dataRef.collection("pages").doc(newPage).collection("style").doc("title").set({
                                                ...
                                                <button className="App-delete" id="Delete"
                                                        onClick={async () => {
                                                            ...
                                                            if(window.confirm("Are you sure you want to delete this page?")) {
                                                                ...
                                                                const res = await dataRef.collection('pages').doc(itemId).delete();
                                        ...
                                        <td>
                                            <div className="slidecontainer">
                                                ...
                                                <input type="text"
                                                       id="Admin-img"
                                                       name = "headerLogo"
                                                       defaultValue={docImg.data().image}
                                                       onChange={(event) => {
                                                           let val = event.target.value;
